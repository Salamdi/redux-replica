...
let store = window.store = Object(__WEBPACK_IMPORTED_MODULE_0_redux__["b" /* createStore */])(__WEBPACK_IMPORTED_MODULE_1__reducers__["a" /* todoApp */]);
...
"use strict";
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_0__actions_js__ = __webpack_require__(25);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_1_redux__ = __webpack_require__(0);




const { SHOW_ALL, SHOW_COMPLETED, SHOW_ACTIVE } = __WEBPACK_IMPORTED_MODULE_0__actions_js__["e" /* visibilityFilters */];

const todos = (state=[], action) => {
    switch (action.type) {
        case __WEBPACK_IMPORTED_MODULE_0__actions_js__["a" /* ADD_TODO */]:
            return state.concat({
                text: action.text,
                completed: false
            });
        
        case __WEBPACK_IMPORTED_MODULE_0__actions_js__["b" /* REMOVE_TODO */]:
            return state.filter((todo, index) => index !== action.index);

        case __WEBPACK_IMPORTED_MODULE_0__actions_js__["d" /* TOGGLE_TODO */]:
            return state.map(
                (todo, index) =>
                action.index === index
                    ? {text: todo.text, completed: !todo.compoleted}
                    : todo
            );
        default:
            return state;
...
const visibilityFilter = (state = SHOW_ALL, action) => {
    if (action.type === __WEBPACK_IMPORTED_MODULE_0__actions_js__["c" /* SET_VISIBILITY_FILTER */]) return action.filter;
    return state;
}
/* unused harmony export visibilityFilter */


const todoApp = Object(__WEBPACK_IMPORTED_MODULE_1_redux__["a" /* combineReducers */])({todos, visibilityFilter});
...
const addTodo = window.addTodo = text => (
    {
        type: ADD_TODO,
        text
    }
...
const removeTodo = window.removeTodo = index => (
    {
        type: REMOVE_TODO,
        index
    }
...
const toggleTodo = window.toggleTodo = index => (
    {
        type: TOGGLE_TODO,
        index
    }
...
const setVisibilityFilter = window.setVisibilityFilter = filter => (
    {
